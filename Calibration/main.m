...
close all
global f_2D 
fileID = fopen('2D_points_calib.txt','w');
fclose(fileID);
fileID = fopen('3D_points_calib.txt','w');
fclose(fileID);
[model msz] = load_model();
[model msz] = load_model();
alpha_sample=zeros(199,1);
beta_sample=zeros(199,1);
I_sample=model2image(model , alpha_sample , beta_sample);
imwrite(I_sample,'sample_image.jpg');
...
f_2D.Resize='off';
hold on
xlim([0 size(I,1)]);ylim([0 size(I,2)])
...
if ~strcmp(c,'q')
    ...
    fileID = fopen('2D_points_calib.txt','a');
...
txt = '';
fileID = fopen('3D_points_calib.txt','a');
